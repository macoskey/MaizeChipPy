...
class FPGA(object):
	...
	# THESE NEED WORK
	def b_set_mask(self,n):
		
		return
	def b_single_channel_mask(self,n):
		
		return
	#################


	def b_execute_program(self,n):
		bcmd = 1
		data = np.array([self.startcode,bcmd,0,0,0,floor(n/256),n % 256,self.endcode,1,1,1,1,1,1,1,1],dtype='uint8')
		self.ser.write(data)
		
	def b_write_chipmem(self,n):
		bcmd = 7
		data = np.array([self.startcode,bcmd,0,n[3],n[2],n[1],n[0],self.endcode,1,1,1,1,1,1,1,1],dtype='uint8')
		self.ser.write(data)
		
	def bgo(self):
		self.b_execute_program(0)

	def bstop(self):
		self.b_stop_execution()
	...
	def a_loadincr_chipmem(self, n,m):
	...
	def write_array_pattern_16bit(self, data):
		...
		for i in xrange(0,len(data),2):
			...
			self.b_write_chipmem(data16bit_tmp)
